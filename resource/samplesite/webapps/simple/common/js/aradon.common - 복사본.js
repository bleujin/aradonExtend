...
var Form = {
  serialize: function(form) {
    var elements = Form.getElements($I(form));
...
Form.Element = {
  serialize: function(element) {
    var felement = $I(element);
    var method = felement.tagName.toLowerCase();
    var parameter = Form.Element.Serializers[method](felement);
  ...
  getValue: function(element) {
    var felement = $I(element);
  	// alert(felement, $I(element), element) ;
    var method = felement.tagName.toLowerCase();
    var parameter = Form.Element.Serializers[method](felement);
...
Date.prototype.format = function(fmt) {
	if (Date.prototype.format.arguments.length == 0) {
		return '' + this.getFullYear() + (this.getMonth() + 1).lpad(2) + this.getDate().lpad(2) ;
	} else {
		alert('babo... not implement');	
	}
} ;
*/
...
$net.ion.util.DomUtil = function(){
	...
	this.getChildByName = function ( htmlElement, name, tagName ){
		...
		throw new IllegalArgumentException( "there is no child element with name" + name );
	} 
...
$net.ion.common.PageOutPut = function(linkName, rowCount, page){
	this.linkName = linkName ? linkName : 'link' ;
    ...
    this.toHtml = function(forwardName, strDepth) {
        ...
        if(this.isValidPage()){
            ...
            return "<script type=\"text/javascript\">" + linkName + ".goListPage(" + linkName + ".getSelfForm().pageNo.value - 1, '" + goForwardName + "' )</script>";
        }
    }
	...
	this.printPageNext = function(head, body, repbody, tail, forwardName) {
		...
		if (this.existNextScreen(page, rowCount)) {
	...
	this.printPageNoList = function(forwardName) {
		...
		if (page.getPageNo() > 1) rtnValue += "<a id=\"n_prev_page\" href=\"javascript:" + linkName + ".goListPage(" + (page.getPageNo() - 1) + ", '" + forwardName + "')\"></a>";
		if (page.getPageNo() < page.getMaxPageNo(rowCount)) rtnValue += "<a id=\"n_next_page\" href=\"javascript:" + linkName + ".goListPage(" + (page.getPageNo() + 1) + ", '" + forwardName + "')\"></a>";
	...
	this.existNextScreen = function(page, rowCount) {
...
$net.ion.aradon.AradonClient = function() {
	...
	this.createLets = function(){
		return new $net.ion.aradon.BatchLet(this) ;
	}
} 


$net.ion.aradon.BatchLet = function(client){
	var client = client ;
	this.lets = new $net.ion.util.Map() ;

	this.addLet = function(name, let) {
		this.lets.put(name, let) ;
	} ;
	
	this.letTojson = function(name) {
		var let = this.lets.get(name) ;
...
$net.ion.aradon.SectionLet = function(section, path){
	...
	this.del = function(sfn){
		...
		// alert(ObjectUtil.toDescString(response, 2, 2)) ;
		return response ;
	} ;
	...
	function init(thisObj){
		...
		$.ajaxSetup({
			...
			error: function(req,error){
				alert('Error', JSON.stringify(req), error) ;
			}, 
