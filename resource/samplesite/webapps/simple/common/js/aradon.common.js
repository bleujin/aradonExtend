...
// $("tempval").innerText = beforeValue ;
	...
	this.goListPage = function goListPage(pageno, forwardName){
		if (this.goListPage.arguments.length >= 2) { // not defined
														// forwardName
	...
	this.goAbsoluteListPage = function goAbsoluteListPage(pageno){// use this
																	// for [from
																	// search to
																	// list]
		try {
	...
	this.goNextScreen = function goNextScreen(pageno, forwardName){
		if (this.goNextScreen.arguments.length >= 2) { // not defined
														// forwardName
	...
	this.goPreScreen = function goPreScreen(pageno, forwardName){
		if (this.goPreScreen.arguments.length >= 2) { // not defined
														// forwardName
			this.goListPage(pageno, forwardName) ;
		} else {
			this.goListPage(pageno) ;
	...
	this.goChangeListNum = function goChangeListNum(textbox, listnum) {
		 if ( (! listnum.toString().isNumeric()) || (listnum - 0 < 1)) { // include
																			// common_lang
	...
	this.goPage = function(pageForward, formId, idValue){
		if (this.goPage.arguments.length == 3 && ! this.isBlank(formId)) { // if
																			// needed
																			// id
			try {
	...
	this.openPage = function(pageForward, formId, idValue){
		if (this.openPage.arguments.length == 3 && ! this.isBlank(formId)) { // if
																				// needed
																				// id
			try {
				this.getSelfForm()[formId].value = idValue ;
	...
	this.actionObj = function actionObj(actionForward, formId, idValue){
		if (this.actionObj.arguments.length == 3 && ! this.isBlank(formId)) { // if
																				// needed
																				// id
			try {
				this.getSelfForm()[formId].value = idValue ;
...
(function(){
  ...
  window.alert=function(q){
    ...
    if (++c%10==0) { // Change the number below to the number of alert boxes
						// to display before the warning is given.
...
/*
 * 
 * Object.prototype.implementsProp=function(propName){ return
 * (this[propName]!=null); } ;
 * Object.prototype.implementsFunc=function(funcName){ return this[funcName] &&
 * this[funcName] instanceof Function; } ;
 * Object.prototype.simpleXmlify=function(tagname){ var xml="<"+tagname; for (i
 * in this){ if (!this[i] instanceof Function){ xml+=" "+i+"=\""+this[i]+"\""; } }
 * xml+="/>"; return xml; } ;
 */

// in explorer, Object can not override
/*
 * Object.prototype.isArray = function() { return (this && this[0]) ? true :
 * false; };
 * 
 * Object.prototype.toJsonString = function(level, depth) { return
 * ObjectUtil.toDescString(this, level, depth) ; };
 */
...
String.prototype.getByteLength = function (){
	...
	for(z=0; this != '' && z < this.length; z++){
		...
		if (c <= 0x0000007F){ // between 0 and 127=> 1byte
			...
			count+= 2;
		} else if (c <= 0x0000FFFF) { // char count increase 3 because common
										// CJK char is 3 bytes
			count+=3 ;
...
String.prototype.isNamoUploadFileNames = function(){  // namo upload file
														// name...
	if(this.length > 0 ){
...
/*
 * -0.01, 10, 10.45 - ok 01, 00.1, .1, 0.0.0 - bad
 */
...
/*
 * isHttpAddress("gosu.pl") - true isHttpAddress("www.gosu.pl") - true
 * isHttpAddress("www.gosu.pl", 1) - false isHttpAddress("https://gosu.pl", 1) -
 * true
 */
...
ObjectUtil._isHTMLElement = function(ele, nodeName) {
  ...
  if (nodeName != null) {
    ...
    // alert("ObjectUtil._isHTMLElement was passed test node name that is
	// neither a string or array of strings");
    return false;
  }
  return true;
};
...
$net.ion.util.Logger = function(loggerLevel, className){
	...
	function print(logLevel, message){
		 ...
		 * elementLog.appendChild(textNode); elementLog.visibility = "visible";
		 */
	}
}
	// all static members
...
$net.ion.common.valid.FormValid = function () {
	...
	this.validate = function(forwardName) {
		...
		for (v=0, vsize=this.validatorList.length ; v < vsize ; v++ ) { // not
																		// use
																		// v,
																		// vsize
																		// in
	...
	this.host = function(fieldName, errorMessage, focus) {
		this.pushValidator(
			new $net.ion.common.valid.RegexValidator(fieldName, /^[a-zA-Z0-9\:\#\$\%\&\=\-\~\+\/\?\_\.\,]+$/, errorMessage, focus));
		// this.pushValidator(new NotAllowedCharValidator(fieldName,
		// "\!\"'\(\)\^\|\\\[\]\{\}`@\*;\<\>", errorMessage, focus)); // ! " ' (
		// ) ^ | \ [ ] { } ` @ * ; < >
	} ;
	...
	this.siteUrl = function(fieldName, errorMessage, focus) { 
		...
		this.pushValidator(new $net.ion.common.valid.RegexValidator(fieldName, /^[a-zA-Z0-9\:\#\$\%\&\*\=\-\~\+\/\?\_\.\,]+$/, errorMessage, focus)); // : #
																																						// $ %
																																						// & *
																																						// = -
																																						// ~ +
...
$net.ion.common.PageOutPut = function(tableId, rowCount, page){
	...
	this.printPageNoList = function() {
		...
		for (var startPage = page.getMinPageNoOnScreen(), endPage = Math.min(page.getMaxPageNo(rowCount), page.getMaxPageNoOnScreen()); startPage <= endPage; startPage++) {
...
$net.ion.aradon.SectionLet = function(section, path){
	...
	this.fullHttp = function(_type, sfn, efn){
		var response = $.ajax({
			type: _type, 
			url: this.fullPath(), 
			data : this.getParams(), 
	...
	function init(thisObj){
		...
		$.ajaxSetup({
			...
			error: function(xhr, stat, err){
				alert('Error', JSON.stringify(xhr), stat, stat.status, err) ;
			}, 
			beforeSend: function(req){
				// this.type = 'POST' ;
				// req.setRequestHeader("MY-HTTP-Method-Override",
				// this.overrideMethod) ;
...
/**
 * TrimPath Template. Release 1.1.2. Copyright (C) 2004 - 2007 TrimPath.
 */
...
(function() {               // Using a closure to keep global namespace clean.
    ...
    TrimPath.parseTemplate_etc.statementDef = { // Lookup table for statement
        ...
        "for"    : { delta:  1, paramMin: 3, 
                     prefixFunc : function(stmtParts, state, tmplName, etc) {
                        ...
                        return [ "var ", listVar, " = ", stmtParts[3], ";",
                             // Fix from Ross Shaull for hash looping, make
								// sure that we have an array of loop lengths to
								// treat like a stack.
                             "var __LENGTH_STACK__;",
                             "if (typeof(__LENGTH_STACK__) == 'undefined' || !__LENGTH_STACK__.length) __LENGTH_STACK__ = new Array();", 
                             "__LENGTH_STACK__[__LENGTH_STACK__.length] = 0;", // Push
																				// a
																				// new
																				// for-loop
																				// onto
																				...
																				// lengths.
                             "if ((", listVar, ") != null) { ",
                             "var ", iterVar, "_ct = 0;",       // iterVar_ct
																// variable,
																// added by B.
																// Bittman
                             "for (var ", iterVar, "_index in ", listVar, ") { ",
                             iterVar, "_ct++;",
                             "if (typeof(", listVar, "[", iterVar, "_index]) == 'function') {continue;}", // IE
																											// 5.x
																											// fix
																											// from
																											// Igor
        ...
        "/for"    : { delta: -1, prefix: "} }; delete __LENGTH_STACK__[__LENGTH_STACK__.length - 1];" }, // Remove
																											// the
																											// just-finished
																											// for-loop
																											// from
    ...
    var parse = function(body, tmplName, etc) {
        ...
        while (endStmtPrev + 1 < body.length) {
            ...
            while (begStmt >= 0) {
                    ...
                    if (blockMarkerEnd >= 0) {
                        ...
                        if (blockEnd >= 0) {                            
                            ...
                            if (blockType == 'cdata') {
                                ...
                                if (blockText != null && blockText.length > 0) // From
																				// B.
																				// Bittman,
																				// eval
																				// should
                ...
                } else if (body.charAt(begStmt - 1) != '@' &&               // Not
																			// an
																			// expression
																			// or
																			// backslashed,
                           body.charAt(begStmt - 1) != '\\') {              // so
																			...
																			// statement
																			// tag.
                    var offset = (body.charAt(begStmt + 1) == '/' ? 2 : 1); // Close
																			// tags
																			// offset
																			// of 2
																			// skips
            ...
            if (begStmt < 0)                              // In "a{for}c",
															// begStmt will be
															// 1.
                break;
            var endStmt = body.indexOf("}", begStmt + 1); // In "a{for}c",
															// endStmt will be
															// 5.
            if (endStmt < 0)
                break;
    ...
    var emitSectionTextLine = function(line, funcText) {
        ...
        while (endExprPrev + endMarkPrev.length < line.length) {
            var begMark = "@{", endMark = "}";
            var begExpr = line.indexOf(begMark, endExprPrev + endMarkPrev.length); // In
																					// "a${b}c",
																					// begExpr
																					// == 1
            if (begExpr < 0)
            ...
            var endExpr = line.indexOf(endMark, begExpr + begMark.length);         // In
																					// "a${b}c",
																					// endExpr
																					// ==
																					// 4;
    ...
    var cleanWhiteSpace = function(result) {
        ...
        // result = result.replace(/^(\s*\S*(\s+\S+)*)\s*$/, '$1'); // Right
		// trim by Igor Poteryaev.
        return result;
    } ;
...
// Otherwise, iterate through all of the keys in the object.
				...
				/*
				 * bleujin for (k in value) {
				 * 
				 * if (Object.hasOwnProperty.call(value, k)) { v = str(k,
				 * value); if (v) { partial.push(quote(k) + (gap ? ': ' : ':') +
				 * v); } } }
				 */
            }
