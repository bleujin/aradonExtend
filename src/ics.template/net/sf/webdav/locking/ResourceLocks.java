...
public class ResourceLocks implements IResourceLocks {
	...
	private static Logger LOG = LogBroker.getLogger(ResourceLocks.class);
	...
	public synchronized boolean lock(ITransaction transaction, String path, String owner, boolean exclusive, int depth, int timeout, boolean temporary) throws LockFailedException {
		...
		if (lo.checkLocks(exclusive, depth)) {
			...
			if (lo.addLockedObjectOwner(owner)) {
				...
				LOG.info("Couldn't set owner \"" + owner + "\" to resource at '" + path + "'");
				return false;
			}
		} else {
			// can not lock
	...
	public synchronized boolean unlock(ITransaction transaction, String id, String owner) {
		...
		if (_locksByID.containsKey(id)) {
			...
			if (_locks.containsKey(path)) {
				...
				LOG.info("net.sf.webdav.locking.ResourceLocks.unlock(): no lock for path " + path);
				return false;
			}
	...
	/**
	 * generates real LockedObjects for the resource at path and its parent folders. does not create new LockedObjects if they already exist
	 * 
	 * @param transaction
	 * @param path
	...
	/**
	 * generates temporary LockedObjects for the resource at path and its parent folders. does not create new LockedObjects if they already exist
	 * 
	 * @param transaction
	 * @param path
	 *            path to the (new) LockedObject
