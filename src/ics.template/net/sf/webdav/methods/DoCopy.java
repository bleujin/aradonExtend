...
public class DoCopy extends AbstractMethod {
	...
	private static Logger LOG = LogBroker.getLogger(DoCopy.class);
	 ...
	 */
	public boolean copyResource(ITransaction transaction, InnerRequest req, InnerResponse resp) throws WebdavException, IOException, LockFailedException {
	...
	/**
	 * copies the specified resource(s) to the specified destination. preconditions must be handled by the caller. Standard status codes must be handled by the caller. a multi status report in case of errors is created here.
	 * 
	 * @param transaction
	 *            indicates that the method is within the scope of a WebDAV transaction
	 * @param sourcePath
	 ...
	 * @throws IOException
	 */
	private void copy(ITransaction transaction, String sourcePath, String destinationPath, Hashtable<String, Integer> errorList, InnerRequest req, InnerResponse resp) throws WebdavException, IOException {
		...
		if (sourceSo.isResource()) {
			...
			long resourceLength = _store.setResourceContent(transaction, destinationPath, _store.getResourceContent(transaction, sourcePath), null, null);
	 ...
	 *             if an error in the underlying store occurs
	 */
	private void copyFolder(ITransaction transaction, String sourcePath, String destinationPath, Hashtable<String, Integer> errorList, InnerRequest req, InnerResponse resp) throws WebdavException {
			...
			for (int i = children.length - 1; i >= 0; i--) {
				...
				try {
					...
					if (childSo.isResource()) {
						...
						long resourceLength = _store.setResourceContent(transaction, destinationPath + children[i], _store.getResourceContent(transaction, sourcePath + children[i]), null, null);
	 ...
	 */
	private String parseDestinationHeader(InnerRequest req, InnerResponse resp) throws IOException {
