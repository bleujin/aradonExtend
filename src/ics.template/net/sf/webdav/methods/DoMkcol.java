...
public class DoMkcol extends AbstractMethod {
	...
	private static Logger LOG = LogBroker.getLogger(DoMkcol.class);
	...
	public void execute(ITransaction transaction, InnerRequest req, InnerResponse resp) throws IOException, LockFailedException {
		...
		if (!_readOnly) {
			...
			if (!checkLocks(transaction, req, resp, _resourceLocks, parentPath)) {
				// TODO remove
				LOG.info("MkCol on locked resource (parentPath) not executable!" + "\n Sending SC_FORBIDDEN (403) error response!");
			...
			if (_resourceLocks.lock(transaction, path, tempLockOwner, false, 0, TEMP_TIMEOUT, TEMPORARY)) {
				...
				try {
					...
					if (parentPath != null && parentSo != null && parentSo.isFolder()) {
						...
						if (so == null) {
							...
							if (so.isNullResource()) {
								...
								if (lockToken.equals(nullResourceLockToken)) {
									...
									// TODO remove
									LOG.info("MkCol on lock-null-resource with wrong lock-token!" + "\n Sending multistatus error report!");
						...
						// TODO remove
						LOG.info("MkCol on non-existing resource is not executable" + "\n Sending SC_NOT_FOUND (404) error response!");
