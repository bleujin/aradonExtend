...
public class DoPropfind extends AbstractMethod {
	...
	private static Logger LOG = LogBroker.getLogger(DoPropfind.class);
	...
	public void execute(ITransaction transaction, InnerRequest req, InnerResponse resp) throws IOException, LockFailedException {
		...
		if (_resourceLocks.lock(transaction, path, tempLockOwner, false, _depth, TEMP_TIMEOUT, TEMPORARY)) {
			...
			try {
				...
				LOG.warning("Sending internal error!");
				resp.sendError(WebdavStatus.SC_INTERNAL_SERVER_ERROR);
	 ...
	 */
	private void recursiveParseProperties(ITransaction transaction, String currentPath, InnerRequest req, XMLWriter generatedXML, int propertyFindType, Vector<String> properties, int depth, String mimeType) throws WebdavException {
	 ...
	 */
	private void parseProperties(ITransaction transaction, InnerRequest req, XMLWriter generatedXML, String path, int type, Vector<String> propertiesVector, String mimeType) throws WebdavException {
